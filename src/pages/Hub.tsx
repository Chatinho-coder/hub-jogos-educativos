...
export default function Hub() {
  ...
  const createGroup = async () => {
    ...
    if (error || !data) { setGroupMsg('Erro ao criar grupo.'); return }
    await supabase.from('group_members').insert({ group_id: data.id, user_id: user.id, user_email: user.email!, role: 'owner' })
    setNewGroupName('')
